...
public class UserApiController {
  ...
  // !!MultipartFile의 변수 이름이 <input>의 name값과 일치해야 함
  public ResponseEntity<?> profileImageUrlUpdate(
    ...
    return new ResponseEntity<>(
        new CMRespDto<>(1,"프로필사진변경 성공",null),
        HttpStatus.OK);
  }
  ...
  public ResponseEntity<?> subscribeList(
    ...
    List<SubscribeDto> subscribeDtos = subscribeService.구독리스트(
        principalDetails.getUser().getId(),pageUserId);
    
    return new ResponseEntity<>(
        new CMRespDto<>(1,"구독자 정보 리스트 불러오기 성공",subscribeDtos),
          HttpStatus.OK);
  }
  ...
  public CMRespDto<?> update(
    ...
    @AuthenticationPrincipal PrincipalDetails principalDetails) { // 세션 정보에 접근
